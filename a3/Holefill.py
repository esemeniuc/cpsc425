            ...
            for i in range(patch_rows):
                for j in range(patch_cols):
                    for k in range(patch_bands):
                        if TODOMask[i][j] == 0:
                            todoCount += 1
                            sum += (float(textureIm[r + i][c + j][k]) - float(TODOPatch[i][j][k])) ** 2

            SSD[r][c] = sum / todoCount
    return SSD


def CopyPatch(imHole: np.ndarray, TODOMask: np.ndarray, textureIm: np.ndarray, iPatchCenter: int, jPatchCenter: int,
              iMatchCenter: int, jMatchCenter: int,
              patchL: int):
    a = selectPatch
    ...
    for i in range(patchSize):
        for j in range(patchSize):
            ...
            val = textureIm[iMatchCenter + i - patchL][jMatchCenter + j - patchL]
            imHole[iPatchCenter + i - patchL][jPatchCenter + j - patchL] = val
...
fillRegion = 'fill_region' + imlabel + '.pkl'
textureRegion = 'texture_region' + imlabel + '.pkl'
if os.path.isfile(fillRegion) and os.path.isfile(textureRegion):
    fill_region_file = open(fillRegion, 'rb')
    fillRegion = pickle.load(fill_region_file)
    fill_region_file.close()

    texture_region_file = open(textureRegion, 'rb')
...
Image.fromarray(imHole).convert('RGB').save(imlabel+'_results.jpg')
